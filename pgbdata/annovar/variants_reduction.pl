...
our $LAST_CHANGED_DATE =	'$LastChangedDate: 2013-07-08 11:10:50 -0800 (Sun, 28 Jul 2013) $';
...
sub filterOperation {
	...
	if ($dbtype eq 'ljb2_sift') {
		my $score_threshold = $ljb2_sift_threshold || 0.05;
		$sc .= " -score_threshold $score_threshold";
	}
	
	if ($dbtype eq 'ljb2_pp2hdiv') {
		my $score_threshold = $ljb2_pp2hdiv_threshold || 0.453;
		$sc .= " -score_threshold $score_threshold -reverse";
	}
	
	if ($dbtype eq 'ljb2_pp2hvar') {
		my $score_threshold = $ljb2_pp2hvar_threshold || 0.447;
		$sc .= " -score_threshold $score_threshold -reverse";
	}
...
=head1 SYNOPSIS
 ...
 Optional arguments:
        ...
        -v, --verbose                   use verbose output
            ...
            --ljb_pp2hvar_threshold <float>	specify the threshold for ljb2_pp2hvar (default: 0.447)
            --ljb_pp2hdiv_threshold <float>	specify the threshold for ljb2_pp2hvar (default: 0.453)
 ...
 Example: variants_reduction.pl infile humandb/ -buildver hg19 -protocol nonsyn_splicing,1000g2012apr_all,esp5400_ea,esp5400_aa,snp135NonFlagged,recessive -operation g,f,f,f,f,m -aaf_threshold 0.01
                  
 Version: $LastChangedDate: 2013-07-08 11:10:50 -0800 (Sun, 28 Jul 2013) $
...
specify the LJB_PP2DIV threshold for filter operation (default: 0.453)

=item B<--ljb_pp2hvar_threshold>

specify the LJB_PP2_HVAR threshold for filter operation (default: 0.447)
